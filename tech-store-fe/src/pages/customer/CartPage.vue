  ...
  <div class="cart-root">
    ...
    <div class="cart-wrap">
      ...
      <div class="cart-header">
        <div>
          ...
          <h1 class="cart-header__title">
            Shopping Cart
            <span v-if="items.length" class="cart-header__count">{{ items.length }}</span>
          </h1>
        </div>
      ...
      <div v-if="loading" class="cart-skeleton">
        <div v-for="i in 3" :key="i" class="cart-skeleton__row" :style="{ '--d': i * 0.08 + 's' }">
      ...
      <div v-else-if="items.length === 0" class="cart-empty">
        <div class="cart-empty__bag">
          <svg width="64" height="64" viewBox="0 0 24 24" fill="none">
            ...
            <path d="M16 10a4 4 0 0 1-8 0" stroke="currentColor" stroke-width="1.2" stroke-linecap="round"/>
          </svg>
        </div>
        <div class="cart-empty__title">Giỏ hàng trống</div>
        <div class="cart-empty__sub">Hãy thêm sản phẩm vào giỏ để tiếp tục</div>
        <button class="btn-shop-now" @click="$router.push('/')">Khám phá sản phẩm</button>
      </div>
      ...
      <div v-else class="cart-content">
        ...
        <div class="cart-items">
          ...
          <transition-group name="item-list" tag="div" class="cart-item-list">
            <div v-for="(row, idx) in items" :key="row.cartItemId" class="cart-item" :style="{ '--delay': idx * 0.06 + 's' }">
              <!-- Product thumb + name -->
              <div class="cart-item__product">
                ...
                <div class="cart-item__info">
                  ...
                  <div v-if="row.variantName" class="cart-item__variant">{{ row.variantName }}</div>
                </div>
              </div>
              ...
              <div class="cart-item__qty">
                <div class="qty-stepper">
                  <button
                    ...
                    :disabled="row.quantity <= 1 || updatingId === row.cartItemId"
              ...
              <div class="cart-item__remove">
                <button
                  class="remove-btn"
                  :class="{ 'remove-btn--loading': removingId === row.cartItemId }"
                  :disabled="removingId === row.cartItemId"
                  @click="removeItem(row)"
                  title="Xóa"
                >
                  <span v-if="removingId === row.cartItemId" class="mini-spinner"></span>
                  <svg v-else width="14" height="14" viewBox="0 0 24 24" fill="none">
                    <polyline points="3 6 5 6 21 6" stroke="currentColor" stroke-width="1.6" stroke-linecap="round"/>
                    <path d="M19 6l-1 14H6L5 6" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"/>
        ...
        <div class="cart-summary">
          <div class="summary-card">
            ...
            <div class="summary-total">
              <span>Tổng cộng</span>
              <span class="summary-total__amount">{{ formatMoney(subtotal) }}</span>
            </div>

            <button class="checkout-btn" :disabled="!items.length" @click="checkout">
          ...
          <div class="trust-badges">
            ...
            <div class="trust-badge">
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none">
                ...
                <path d="M20.49 15a9 9 0 1 1-2.12-9.36L23 10" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"/>
              </svg>
              Đổi trả dễ dàng
            </div>
          </div>
...
:root {
  ...
  --green:    #1a7a4a;
  --radius:   16px;
  --shadow:   0 2px 16px rgba(15,10,5,0.07), 0 1px 3px rgba(15,10,5,0.05);
  --shadow-lg:0 8px 40px rgba(15,10,5,0.10);
}
...
.back-btn:hover { color: var(--text); border-color: rgba(15,10,5,0.15); transform: translateX(-2px); }

/* ── Skeleton ── */
.cart-skeleton { display: flex; flex-direction: column; gap: 3px; }

.cart-skeleton__row {
  display: grid; grid-template-columns: 2fr 1fr 1.2fr 1fr 1.2fr auto;
...
.sk--img   { width: 56px; height: 56px; border-radius: 10px; }
.sk-lines  { display: flex; flex-direction: column; gap: 8px; }
.sk--long  { height: 13px; width: 85%; }
.sk--short { height: 10px; width: 55%; }
.sk--price { height: 13px; width: 70%; }
.sk--qty   { height: 34px; width: 100px; border-radius: 10px; }
.sk--sub   { height: 13px; width: 80%; }
.sk--btn   { height: 32px; width: 36px; border-radius: 8px; }
...
.cart-empty__bag {
  color: rgba(15,10,5,0.15); margin-bottom: 20px;
}
...
.cart-empty__sub { font-size: 14px; color: var(--muted); margin-bottom: 28px; }

.btn-shop-now {
  padding: 12px 28px; border-radius: 100px;
  background: linear-gradient(135deg, #c9922b, #b07820);
  color: white; border: none;
  font-family: 'Plus Jakarta Sans', sans-serif;
  font-size: 14px; font-weight: 600;
  cursor: pointer; box-shadow: 0 4px 16px rgba(201,146,43,0.3);
  transition: transform 0.15s, box-shadow 0.15s;
}

.btn-shop-now:hover { transform: translateY(-2px); box-shadow: 0 8px 24px rgba(201,146,43,0.35); }
...
.cart-table-head {
  ...
  padding: 13px 22px;
  background: linear-gradient(to bottom, #faf6f0, #f5f0e8);
...
.cart-item:hover { background: #faf6f1; }
...
.item-list-enter-active { transition: all 0.35s cubic-bezier(0.34, 1.2, 0.64, 1); }
.item-list-leave-active { transition: all 0.25s ease; }
.item-list-enter-from   { opacity: 0; transform: translateX(-12px); }
.item-list-leave-to     { opacity: 0; transform: translateX(12px); height: 0; padding: 0; }
...
.cart-item__variant { font-size: 11.5px; color: var(--muted); margin-top: 3px; }
...
.qty-stepper__btn:hover:not(:disabled) { background: rgba(201,146,43,0.12); color: var(--gold); }
...
.remove-btn {
  ...
  background: #fff5f5; border: 1px solid rgba(239,68,68,0.15);
...
.remove-btn:hover:not(:disabled) {
  background: #fee2e2; border-color: rgba(239,68,68,0.3);
  transform: scale(1.08);
}

.remove-btn:disabled { opacity: 0.5; cursor: not-allowed; }
.remove-btn--loading { opacity: 0.6; }
...
.summary-line--muted span { color: var(--muted); font-size: 13px; }
...
.checkout-btn {
  ...
  background: linear-gradient(135deg, #1a1410, #3b2a1a);
  color: white;
  font-family: 'Plus Jakarta Sans', sans-serif;
  font-size: 15px; font-weight: 700;
  cursor: pointer; display: flex; align-items: center;
  justify-content: center; gap: 9px;
  box-shadow: 0 6px 24px rgba(15,10,5,0.2);
  transition: transform 0.15s, box-shadow 0.15s;
...
.checkout-btn:hover:not(:disabled) {
  ...
  box-shadow: 0 10px 32px rgba(15,10,5,0.25);
}
...
.checkout-btn__shimmer {
  ...
  transform: translateX(-100%); transition: transform 0.65s;
}

.checkout-btn:hover .checkout-btn__shimmer { transform: translateX(100%); }
...
.continue-btn:hover { background: #f5f0e8; color: var(--text); border-color: rgba(15,10,5,0.12); }
...
@media (max-width: 640px) {
  ...
  .cart-item__subtotal { font-size: 13px; }
}
</style>